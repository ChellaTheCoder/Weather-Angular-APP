...
export class WeatherService {
  ...
  getWeatherByСurrentLocation(): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
        appConfig.defaultCity.map((cityE) => {
          this.subscribers.city = this.getWeatherByCity(cityE.city).subscribe((weather) => {
      ...
      .switchMap(() => {
        return this.http.get(`${apiConfig.host}/weather?appid=${apiConfig.appId}&q=${city}&units=${this.unitSystem}`)
          ...
          .map((data) => {
            const weather = this.handleResponseWeatherData(data);
            this.weather.next(weather);
            appConfig.weatherArray.push(weather);
            return appConfig.weatherArray;
          })
          .catch(this.handleError);
      }
      );
